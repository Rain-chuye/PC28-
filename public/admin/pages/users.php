...
<body>
    <header class="admin-header">
        <div class="px-6 py-4 bg-white border-b border-slate-100 flex gap-3">
        <div class="flex-1 bg-slate-50 rounded-xl px-4 flex items-center gap-3 border border-slate-100">
            <i class="fas fa-search text-slate-300 text-xs"></i>
            <input type="text" id="user-search" class="w-full bg-transparent border-none outline-none py-3 text-xs font-bold" placeholder="搜索 ID, 用户名 or 昵称..." onkeyup="if(event.keyCode==13) loadUsers()">
        </div>
        <button onclick="loadUsers()" class="w-12 h-12 bg-indigo-600 text-white rounded-xl flex items-center justify-center shadow-lg shadow-indigo-100"><i class="fas fa-sync-alt"></i></button>
    </div>
        <h1>会员管理</h1>
    ...
    <script>
        ...
        async function loadUsers() {
            try {
                const search = document.getElementById('user-search').value; const res = await fetch('/admin/api/users_list.php?search=' + encodeURIComponent(search)).then(r => r.json());
